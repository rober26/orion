...
export default function ProjectSidebar() {
  ...
  if (collapsed) {
    return (
      <>
        <aside className="h-full w-14 shrink-0 border-r border-orion-border dark:border-orion-dark-border bg-slate-50/60 dark:bg-slate-900/20">
          <div className="flex h-full flex-col items-center py-3">
            <button
              type="button"
              onClick={() => setCollapsed(false)}
              className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800/60"
              aria-label="Expandir menu del proyecto"
            >
  ...
  return (
    <>
      <aside className="h-full w-72 shrink-0 border-r border-orion-border bg-slate-50/60 dark:border-orion-dark-border dark:bg-slate-900/20">
...
function SidebarContent({
  ...
  pathname: string;
  onDocumentOpen: (doc: ProjectDocument) => void;
  onNavigate?: () => void;
  onCollapse?: () => void;
}) {
      ...
      <div className="border-b border-orion-border p-4 dark:border-orion-dark-border">
        <div className="mb-2 flex items-center justify-between">
          <button
            ...
            className="inline-flex min-h-9 items-center gap-2 rounded-xl px-2 text-xs font-bold uppercase tracking-widest text-slate-500 transition-colors hover:bg-slate-100 hover:text-orion-primary dark:text-slate-300 dark:hover:bg-slate-800/60"
          >
          ...
          {onCollapse ? (
            <button
              type="button"
              onClick={onCollapse}
              className="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800/60"
              aria-label="Colapsar menu del proyecto"
            >
        ...
        <div className="mt-3 space-y-2">
          <button
            ...
            className="w-full truncate rounded-xl px-2 py-1 text-left text-base font-black text-slate-900 transition-colors hover:bg-slate-100 hover:text-orion-primary dark:text-white dark:hover:bg-slate-800/60"
            title={projectName}
          >
            {projectName}
          </button>
