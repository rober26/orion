      ...
      <header className="surface-panel sticky top-0 z-40 w-full shrink-0 rounded-none border-x-0 border-t-0 bg-white/85 px-3 py-2 backdrop-blur-sm sm:px-4 lg:px-6 dark:bg-slate-900/85">
        <div className="relative flex items-center gap-3">
          ...
          >
            ...
            className="icon-btn rounded-xl bg-slate-100 text-slate-700 hover:bg-slate-200 lg:hidden dark:bg-slate-800 dark:text-slate-100 dark:hover:bg-slate-700"
            ...
            >
              ...
              >
                <nav className="grid grid-cols-2 gap-2">
                  {navigationTargets.map((item, index) => {
                    const isActive = item.href === "/" ? pathname === "/" : pathname.startsWith(item.href);
                    return (
                      <Link
                        key={item.href}
